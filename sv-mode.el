...
(defconst sv-font-lock-keywords-re
  (regexp-opt '("module"
                "endmodule")
              'words))

(defconst sv-font-lock-keywords
  (list
   ;; (cons sv-font-lock-keywords-re 'font-lock-keyword-face))
   ;; `(,sv-font-lock-keywords-re . font-lock-keyword-face))
   `(,sv-font-lock-keywords-re . font-lock-builtin-face))
  "SystemVerilog Keywords for highlight.")
...
(defvar sv-mode-map
  (let ((map (make-keymap)))
    ...
    map)
  "Key map for SystemVerilog major mode.")
...
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.mysv\\'" . sv-mode))
